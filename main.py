...
def load_user(user_id):
    user = dbHandler.get_user_by_column("xpp9x_id", user_id)
    if user:
...
def login():
    ...
    if form.validate_on_submit():
        ...
        user = dbHandler.get_user_by_column("K3hq4_email", email)
        if user and check_password_hash(user.password, password):
            session['pre_2fa_user_id'] = user.id
            return redirect(url_for('two_factor'))
        else:
        # Flash message if login fails.
        # Small delay for noise.
            time.sleep(1)
...
def register():
    ...
    if form.validate_on_submit():
        ...
        existing_user = dbHandler.get_user_by_column("K3hq4_email", email)
        if existing_user:
        ...
        # Generate an API key for API integration
        ...
        dbHandler.insert_user(email, hashed_password, otp_secret, api_key, api_key_expiration, name)
        flash('Registration successful! Please set up 2FA.', 'success')
        session['pre_2fa_user_id'] = dbHandler.get_user_by_column("K3hq4_email",email).id
...
def delete_entry(entry_id):
    ...
    entry = dbHandler.get_diary_by_id(entry_id)
    if entry and entry[10] == current_user.id:
        dbHandler.delete_diary(entry_id)
